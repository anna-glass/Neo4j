...
RETURN 
  apoc.map.merge(properties(f), {labels: labels(f)}) AS person1, 
  apoc.map.merge(properties(p), {labels: labels(p)}) AS person2, 
...
RETURN 
  apoc.map.merge(properties(f1), {labels: labels(f1)}) AS person1, 
  apoc.map.merge(properties(f2), {labels: labels(f2)}) AS person2, 
